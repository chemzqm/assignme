...
exports.list = function* () {
  ...
  query = query.order(user.updateAt.descending).toQuery();
  var res = yield this.db.query(query.text, query.values);
  if (res && res.length > 0) {
...
exports.post = function* () {
  ...
  var query = user.select(user.id).from(user)
          .where(user.email.equals(u.email)).toQuery();
  var res = yield this.db.query(query.text, query.values);
  if (res[0].length > 0) this.throw('邮箱已存在', 406);
  u.password = cryptoPassword(u.password);
  if (!u.icon) {
  //默认 gravatar图片
    u.icon = gravatar.url(u.email, {s: '100', r: 'pg', d: 'retro'}, true);
  }
  query = user.insert(u).toQuery();
